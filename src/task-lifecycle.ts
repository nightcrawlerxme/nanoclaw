...
export function advanceTaskLifecycle(
  taskId: string,
  now?: Date,
  ...
  if (currentState === 'born') {
    ...
    if (task.last_run !== null) {
      ...
      if (nowMs - lastRunMs <= LIFECYCLE_THRESHOLDS.stalledAfterMs) {
        nextState = 'active';
      } else if (nowMs - lastRunMs > LIFECYCLE_THRESHOLDS.dyingAfterMs) {
        nextState = 'dying';
      }
    }
    ...
    if (task.last_run !== null) {
      ...
      if (nowMs - lastRunMs <= LIFECYCLE_THRESHOLDS.stalledAfterMs) {
        nextState = 'active';
      } else if (nowMs - lastRunMs > LIFECYCLE_THRESHOLDS.deadAfterMs) {
        nextState = 'dead';
      }
    }
  ...
  db.prepare('UPDATE scheduled_tasks SET lifecycle_state = ? WHERE id = ?').run(
    nextState,
    taskId,
  );

  logger.info(
    { taskId, from: currentState, to: nextState },
...
export function getTaskFossil(id: string): TaskFossil | undefined {
  const db = getDb();
  const row = db.prepare('SELECT * FROM task_fossils WHERE id = ?').get(id) as
    | (Omit<TaskFossil, 'context_snapshot'> & {
...
export function getTaskFossils(groupFolder: string): TaskFossil[] {
  ...
  const rows = db
    .prepare(
      'SELECT * FROM task_fossils WHERE group_folder = ? ORDER BY fossilized_at DESC',
    )
    .all(groupFolder) as (Omit<TaskFossil, 'context_snapshot'> & {
    context_snapshot: string | null;
  })[];
...
export function startLifecycleMonitorLoop(options?: {
    ...
    try {
      ...
      for (const task of tasks) {
        try {
          ...
          logger.error(
            { taskId: task.id, err },
            'Error advancing task lifecycle',
          );
        }
      }
    } catch (err) {
